...
void main() {
  ...
  group('Getters on Duration extension', () {
    test('fuzzy getter (long form)', () {
      expect(const Duration(minutes: 5).fuzzy, 'about 5 minutes');
    });

    test('fuzzyShort getter (short form)', () {
      expect(const Duration(minutes: 5).fuzzyShort, '~5 min');
    });
  });
  ...
  group('General coverage', () {
    ...
    test('0 ms (exact now)', () {
      ...
      expect(Duration.zero.fuzzyTime(form: FuzzyForm.short), 'now');
    });
  });
